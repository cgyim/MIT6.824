...
func doMap(
	...
	fdSlice := make([]*os.File, nReduce, nReduce)
	fdEncoderSlice := make([]*json.Encoder, nReduce, nReduce)
	fdInfile, err  := os.Open(inFile)
	defer fdInfile.Close()
	if err != nil {
		panic("open infile error")
	}
	content , err := ioutil.ReadAll(fdInfile)
	if err != nil {
		panic("can not read infile content")
	}
	...
	for i := 0; i < nReduce ; i++ {
		...
		if err != nil {
			panic("open file for " + filename + "error")
		}
		fdSlice[i] =  fd
		fdEncoderSlice[i] = json.NewEncoder(fd)
	}
	for _ , kv := range kvSlice {
		fdEncoderSlice[ihash(kv.Key) % nReduce].Encode(&kv)
	}
	for _, eachFd := range fdSlice {
		eachFd.Close()
	}
